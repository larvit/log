...
and on React Native, whose `fetch` is an `XMLHttpRequest` polyfill, may send them instead. Neither
the username nor the password reaches the span: `url.full` drops the userinfo, and the rejection
quoting the url reaches the status message as `http://REDACTED@host/x`.
